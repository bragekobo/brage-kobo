...
(function (root) {
  ...
  /* ============================================================
     ...
     ============================================================ */
  /* ★★ T130・社長の ご指示：「盤が デカすぎるので、少し 小さくして」★★
     ------------------------------------------------------------
     ★ パソコンでは 盤の たてが 天井に なる ので、1マスが 126px まで ふくらんで いました
       【実測 1512×945：盤（青い わくの 外）904×778px ＝ 画面の たての 82%／1コマ 103px】。
     ★ そこで **1マスに 上限（CELL_MAX）** を 1つだけ 置きます。
       ★ 上限を 決めた ものさしは 会社の 中に すでに あります ――
         ★★ **大富豪の パソコン札 83 × 126px**（T119 で 決め、社長が「これで いい」と された 大きさ）。
         ★ コマ ＝ 札の はば 83px に そろえる → cell = 83 ÷ 0.82 ≒ 101px。
         ★ round(101 × 0.82) = 83px ―― ★ぴたり 合います。
     ★ 「小さくしすぎ」の 下限も 数字で 置きます（→ 報告 §2）：
       ★ 320pxスマホの 1マス 43px は、実寸 6.97mm を 約300mm 先で 見る ＝ 1.33°。
       ★ パソコン（1px≒0.20mm・約550mm 先）で 同じ 見かけに なるのは **64px**。
       ★★ つまり cell 64px を 割ると、パソコンの 盤は **いちばん 小さい スマホより 見づらく** なります。
       ★ 101px は その 下限の 1.58倍。★目が 悪い人の 話（T119・T123）を 踏まえて、
         下限すれすれには 寄せません。
     ⚠️★ スマホ・横向きは はば で 決まって いる ので（50／43／42px）、この 上限は かかりません。
        ★ 1pxも 動きません【実測で 確認・T130】。 */
  var DIM = {
    ...
    CELL_MAX: 101,     // ★ T130：これ以上 大きく しない（＝ コマ 83px ＝ 大富豪の PC札の はば）
  ...
  function fitBoard(W, H, cols, rows) {
    ...
    /* ★ T130：上限（CELL_MAX）を かけてから 下限（CELL_MIN）で 受ける。
       ★ 上限が 先。★でないと 器が うんと 小さい とき、上限が 下限を 押し戻して しまう。 */
